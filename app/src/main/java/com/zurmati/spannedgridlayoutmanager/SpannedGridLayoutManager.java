...
public class SpannedGridLayoutManager extends RecyclerView.LayoutManager {
    ...
    private int getExactBottom() {
        int bottom = getDecoratedBottom(getChildAt(getChildCount() - 1));
        for (int index = columns; index > 0; index--) {
            if (getChildCount() - index != -1) {
                int bottomSecondary = getDecoratedBottom(getChildAt(getChildCount() - index));
                Log.i("MyCalculationsNow", "scrollVerticallyBy: bottom " + bottom + " bottom Secondary " + bottomSecondary);
                bottom = Math.max(bottom, bottomSecondary);
            }
        }
        return bottom;
    }
